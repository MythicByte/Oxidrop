...
impl NamespaceTopology {
    fn create() -> Self {
        ...
        netns(
            &client,
            &["-6", "route", "add", "fd00:2::/64", "via", "fd00:1::1"],
        );
        netns(
            &server,
            &["-6", "route", "add", "fd00:1::/64", "via", "fd00:2::1"],
        );
...
fn firewall_rule_v6(input: &str, output: &str, source: &str, destination: &str, add: bool) {
    let action = if add { "-I" } else { "-D" };
    let mut args = vec![action, "FORWARD"];
    if add {
        args.push("1");
    ...
    let status = Command::new("ip6tables")
        .args(&args)
        .status()
        .unwrap_or_else(|error| panic!("failed to execute ip6tables: {error}"));
    if add {
        assert!(
            status.success(),
            "ip6tables {action} FORWARD rule failed for {input}->{output}"
        );
    } else if !status.success() {
        eprintln!("warning: ip6tables cleanup failed for {input}->{output}");
    }
}
...
fn ping6(namespace: &str) -> std::process::Output {
    Command::new("ip")
        .args([
            "netns",
            "exec",
        ...
        .output()
        .unwrap_or_else(|error| panic!("failed to execute IPv6 ping in {namespace}: {error}"))
}
...
async fn real_veth_namespaces_forward_and_deny_stateful_icmp() {
    ...
    let client_v6 = [0xfd00_0001_u32, 0, 0, 2];
    let server_v6 = [0xfd00_0002_u32, 0, 0, 2];
    subnet_v4
    ...
    subnet_v6
        .insert(
            &Key::new(128, ipv6_network_bytes(client_v6)),
            Action::Allow,
            0,
        )
        .expect("IPv6 client subnet must be allowed");
    subnet_v6
        .insert(
            &Key::new(128, ipv6_network_bytes(server_v6)),
            Action::Allow,
            0,
        )
        .expect("IPv6 server subnet must be allowed");
    allow_list_v4
        .insert(
    ...
    allow_list_v6
        .insert(
            Ipv6Packet::new(server_v6, client_v6, 0, 0, 58),
            AllowListState {
                action: Action::Allow,
                last_seen: 0,
            },
        ...
        .expect("ICMPv6 flow must be allowed");
    allow_list_v6
        .insert(
            Ipv6Packet::new(client_v6, server_v6, 0, 0, 58),
            AllowListState {
                action: Action::Allow,
                last_seen: 0,
            },
        ...
        .expect("reverse ICMPv6 flow must be allowed");
    ...
    let ping6_output = ping6(&topology.client);
    assert!(
        ping6_output.status.success(),
        "allowed ICMPv6 must cross the routed namespace topology; stdout={}; stderr={}",
        String::from_utf8_lossy(&ping6_output.stdout),
        String::from_utf8_lossy(&ping6_output.stderr)
    );
    ...
    allow_list_v6
        .remove(&Ipv6Packet::new(server_v6, client_v6, 0, 0, 58))
        .expect("ICMPv6 flow state must be removable");
    allow_list_v6
        .remove(&Ipv6Packet::new(client_v6, server_v6, 0, 0, 58))
        .expect("reverse ICMPv6 flow state must be removable");
    ...
    let denied_ping6 = ping6(&topology.client);
    assert!(
        !denied_ping6.status.success(),
        "ICMPv6 must fail after its stateful allow-list entries are removed; stdout={}; stderr={}",
        String::from_utf8_lossy(&denied_ping6.stdout),
        String::from_utf8_lossy(&denied_ping6.stderr)
    );
}
